...
/**
 * Build a minimal DOM matching the poster/ticker structure used by
 * community.astro and EventsPoster.astro. Returns the document root so tests
 * can call expireEvents against the real exported function.
 */
function buildDOM({
  headlinerExpires,
  rowExpires,
...
describe('expireEvents: headliner expired, rows remain → list--bare added', () => {
  it('adds list--bare to .list when headliner expired but row events remain', () => {
    ...
    // Evergreen must stay hidden — there are still upcoming events.
    const evergreen = doc.getElementById('poster-evergreen')
